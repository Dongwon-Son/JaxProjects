...
def get_spherical_harmonics(l, theta, phi):
    ...
    """
    return np.stack([ get_spherical_harmonics_element(l, m, theta, phi) \
                         for m in range(-l, l+1) ],
                        axis = -1)


def pnts_to_sh_theta_phi(pnt):
    '''
    pnt => beta, alpha of ZYZ euler with relative coordinate
    '''
    theta = np.arccos(pnt[...,2]/np.linalg.norm(pnt))
    phi = np.arctan2(pnt[1], pnt[0])
    return theta, phi

def get_spherical_harmonics_from_ZYZ(l, alpha, beta):
    return get_spherical_harmonics(l, np.pi-beta, alpha)

def get_spherical_harmonics_from_pnts(l, pnt):
    beta, alpha = pnts_to_sh_theta_phi(pnt)
    return get_spherical_harmonics(l, np.pi - beta, alpha)
