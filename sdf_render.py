...
# %%
def sdf_renderer(sdf_func, cam_SE3, color, background_color=jnp.array([1,1,1])):
    NS = 50
    PIXEL_SIZE =  [50, 50]
    def vec_normalize(vec):
        return vec/(jnp.linalg.norm(vec, axis=-1, keepdims=True) + 1e-8)
...
img = sdf_renderer(lambda x : sphere_sdf(np.array([0,0,0]), 0.4, x), cam_SE3, color=jnp.array([0,0.5,0]))
